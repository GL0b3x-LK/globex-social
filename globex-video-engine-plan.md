...
| `voice_id` text | ElevenLabs voice, created once via Voice Design, then **locked forever** — mechanically kills "mid-video accent shifts" |
| `reference_image_urls` text[] | 3–5 canonical images (front, 3⁄4, waist-up, in-context), Supabase Storage |
...
- **Voice: ElevenLabs Starter ($6/mo)** — commercial license, Voice Design for the 10 invented personas + 1 narrator, character-level timestamps endpoint for caption timing. Voices created once, ids locked on the character row. Fallback: Cartesia Pro ($5/mo).
...
- [ ] **Supply an image key** (`KIE_API_KEY` is empty) → generate the 10 sheets → Ilan/Len one-time approval → flip each to `approved` → create voices
- [ ] ElevenLabs account; Epidemic subscription + first 12-track music pool
