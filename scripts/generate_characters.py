...
_CONCURRENCY = 4  # kie.ai rate-limits; keep the fan-out polite
_JPEG_QUALITY = 88
...
SHOTS: tuple[Shot, ...] = (
    ...
    Shot(
        ...
        "hands visible, environment readable behind them",
        "9:16",
    ),
    Shot(
        "three_quarter",
...
async def _one(
    character: library.Character, shot: Shot, sem: asyncio.Semaphore, *, force: bool
) -> tuple[str, bool]:
    ...
    if path.exists() and not force:
        ...
        return label, True
    async with sem:
        result = await image_gen.generate(build_prompt(character, shot), aspect_ratio=shot.aspect)
...
async def run(characters: list[library.Character], shots: tuple[Shot, ...], *, force: bool) -> int:
    sem = asyncio.Semaphore(_CONCURRENCY)
    results = await asyncio.gather(
        *(_one(c, s, sem, force=force) for c in characters for s in shots)
    )
...
def main() -> int:
    ...
    return asyncio.run(run(characters, shots, force=args.force))
