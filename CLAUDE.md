...
- **Templates are the default; AI image generation is the photographic layer ONLY (updated 2026-06-04).** HTML/CSS templates remain the default and guarantee pixel-perfect brand output. AI image generation (kie.ai / nano-banana) is now allowed, but ONLY as the photographic background *underneath* the brand template overlay — the logo, Pantone navy/cyan, ring motif, and headline are still rendered by the template on top, so brand identity is never left to the generative model. A generated image flows through the exact same path as a user-supplied photo (`render_and_store(photo_bytes=…)` → an approved template). Never post a raw generated image without the brand overlay.
- **Client-facing posts use the four approved templates ONLY (July 30 2026 sign-off).** `TS-p1-bolddip_4x5`, `TS-p2-cut-navyborder_4x5`, `TS-p3-editorial_4x5`, `MS-3-anniv-photo_4x5` — see `catalog.FINAL_VARIANTS`. They render from `_final.html` in **Poppins** (MS-3 overrides to **Inter**, with Archivo Black for the years badge). Everything else in `TEMPLATES` is demo-era, renders from `_base.css` in **Montserrat**, and must never front a client post: the free-form prompt offers only the four, an unknown variant resolves to `catalog.DEFAULT_FINAL`, and an attached photo on a text-only template switches to a final rather than to `custom`. All four are built around a photograph, so a post that reaches one without an image is given one from the asset bank rather than dropped onto a non-approved template. Generation is opt-in (Karen explicitly asks, or the agent asks when unsure) — text/number posts stay typographic.
