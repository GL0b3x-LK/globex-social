...
### 2026-08-09 — Deployed to production + calendar held dormant until a launch date
- **Railway (client's own account):** CLI moved off Mike's login to **karen@globexusa.com** (workspace gl0b3x-lk). Project **lucky-possibility** → service **globex-social**; it had ZERO app variables, so all 15 were set from `.env` (ENVIRONMENT=production). Deployed the Docker image; `/health` returns 200 `{supabase: connected, anthropic: configured}` at **https://globex-social-production-3162.up.railway.app**.
- **Guided intake shipped** (3rd commit): bare "new post" → 4 questions (about / why / picture / template) → preview → approval. Substantial briefs and attached media keep the one-shot fast path.
- **Launch-date decoupling (important):** the scheduler only looks forward, so every day spent un-launched silently dropped that day's post. Fixed by making the schedule **derived, not stored**: `calendar.json` now carries `planned_date` + an `anchored` flag; `calendar_source.schedule()` returns nothing at all while `CALENDAR_LAUNCH_DATE` is unset (dormant, zero risk), and re-flows the whole year when it's set — **59 anchored** entries (holidays / shows / anniversaries / 12 season-tied fillers) keep their real dates and drop only if already past; **97 floating** evergreen posts refill the remaining M/W/F slots in approved order, so they're never lost. Event ids are launch-independent, so re-anchoring can't re-draft an already-previewed post.
- **`scripts/set_launch_date.py`** is the go-live switch: preview any date (shows the first fortnight and exactly what would be skipped), then `--apply` writes CALENDAR_LAUNCH_DATE + SCHEDULER_ENABLED=true to Railway. Sample: a 7 Sep launch schedules 155/156, dropping only "National Poultry Month kickoff" (31 Aug).
- **Tests:** 193 passed / 47 skipped; mypy clean across all 72 source files.
- **Blocking launch (all client-side):** Twilio webhook must be pointed at `/webhooks/twilio/message` (console-only, no API); Karen's number added FIRST in `AUTHORIZED_NUMBERS` (first entry = approval recipient); real IG/FB/LinkedIn connected in Blotato (only "globextest" IG today — fine for the dummy-account demo Len asked for); production WhatsApp sender (sandbox now, Meta verification = longest lead).

### 2026-08-09 — v2 build: final templates rebuilt + Phase 6 (calendar scheduler) landed
- **Context:** calendar green-lit; contracted build started. Two commits: template rebuild, then scheduler.
- **The 4 approved templates** (TS-p1-bolddip, TS-p2-cut-navyborder, TS-p3-editorial, MS-3-anniv-photo) rebuilt as parametric HTML/CSS, pixel-calibrated against the reference PNGs (side-by-side + ink-box measurement loops). New `_final.html` design system: website colors **#002D70/#5BC0DE** (Pantone conversions purged repo-wide incl. CLAUDE.md), 1080×1350 portrait canvas @2x = 2160×2700.
- **Font forensics surprise:** the finals mix THREE typefaces — Inter (TS-p2 + MS-3), Poppins (TS-p3), Comfortaa (TS-p1; the curled 'l' gave it away). None of them Montserrat (demo-era). All self-hosted woff2, sizes/tracking solved per element via a Chromium canvas measureText probe against reference ink boxes.
- **Phase 6 built** (was the last unbuilt phase, now ahead of plan): green-lit 52-week calendar shipped into `app/data/calendar.json` (156 posts, tests assert uniqueness + 4-template whitelist); curated 17-asset photo pool (`app/data/asset_pool/`) with tag-matched, deterministic photo picking (branded packaging outranks raw-meat shots — Len's taste); `app/workflows/scheduled.py` drafts due entries (lead window, idempotent via uuid5 event ids on the posts table — no new DDL needed, works without SUPABASE_DB_URL), sends previews through the SAME `_finalize_preview` → approval flow as the command center. **Approved scheduled posts hold until their calendar date** (`render_meta.publish_on` gate in `handle_approval`) and `publish_due_posts` fires them on the day + notifies the approver. APScheduler wired into FastAPI lifespan, gated by `SCHEDULER_ENABLED` (off in dev/tests).
- **Render bridge** now maps calendar template names + GeneratedPost fields onto the finals' slots (middot subheads split into the approved two-tone dateline) and picks each template's canvas from the catalog.
- **E2E dry run** (real Opus + real render, stubbed DB/Twilio): 4 categories drafted correctly with on-brief copy; visual QA led to the packaging-over-raw photo bias.
- **Tests:** 175 passed / 47 skipped (was 150/46). New `tests/test_scheduled.py` (14).
- **Queued next:** command-center 4-question flow (operator-guided drafting); approval-gating + publish-job unit tests; Blotato real-account connections (only "globextest" IG connected); production WhatsApp sender (currently sandbox — longest lead time); Railway project (none exists on the connected account — Sonder projects only); Higgsfield workspace switch before image-gen goes live.
- **What landed:** approval now **really publishes** (the stub is gone). `app/publishing/blotato.py` `BlotatoClient` — resolves connected accounts dynamically (`GET /v2/users/me/accounts`, cached), publishes one platform per call (`POST /v2/posts` → poll `GET /v2/posts/{id}` until `published`/`failed`), retries 429/5xx/network (not 4xx), FB `pageId` required / LinkedIn optional, media passed as a public URL (Supabase). `formatter.py` (per-platform caption + length trim IG 2200 / FB 63206 / LI 3000). `publisher.publish_post` publishes to **target ∩ connected**, writes a `post_platforms` row per platform, marks the post `published` if ≥1 succeeded (else stays `approved` for the Phase 6 retry job), and returns per-platform results. `handle_approval` sends Karen the real status: *"Published: Instagram ✓ · LinkedIn ✗"*.
- **Per-post platform targeting (the requested add):** "post this **only to LinkedIn**" works. `app/publishing/platforms.py` — deterministic parser (synonyms ig/insta, fb, linkedin/linked in; "all"/"everywhere" resets; naming = exclusive). New `posts.target_platforms text[]` column (null = all). Captured at **create / edit / approval**; the preview shows *"📲 Posting to: LinkedIn only"*; publish honors it. Only Instagram is currently connected on the Blotato test account (`globextest`) — unconnected targeted platforms are reported as "not connected" rather than silently dropped, and FB/LinkedIn light up automatically once linked in the Blotato dashboard (dynamic resolution, nothing hardcoded).
- **Acceptance PASS:** ruff + mypy clean (67 src files); full suite **150 passed / 46 skipped** — `test_platforms.py` (8), `test_formatter.py` (4), `test_blotato.py` (respx: success / not-connected / partial / failed-status / retry-503 / no-retry-401), + 3 e2e (targeting captured & shown, default untouched, approval status). **Verified live (read-only):** Blotato auth + `GET /users/me/accounts` returns the connected IG account.
- **Deferred to a live smoke (not auto-run):** an actual end-to-end publish to the `globextest` IG — it's a real public post, so left for Abdul to trigger, not auto-fired. Real video publishing works through the same path (media_url can be the VHS mp4).
- **Queued next:** Phase 6 (Automation — daily scheduler + branded-packaging rotation + the `retry_failed_publishes` job that drains `approved`-but-unpublished posts).

### 2026-06-04 — Iteration: 3 new templates (Polaroid, Quote Card, VHS video)
- **What landed:** three client-approved templates, one per category. **Polaroid** (image) — a snapshot (Karen's photo or a generated image) in a white Polaroid frame on a Globex-navy field, caption from the headline; photo-capable (added to render_pipeline `_PHOTO_CAPABLE`), G-man fallback when no photo. **Quote Card** (static) — a short quote as a clean navy typographic card (cyan quote-mark, crisp white text via the template — no AI-image gibberish). Both are normal HTML/CSS catalog templates, offered to the model in the freeform prompt, keep the brand-footer logo, and pass the per-template brand-palette audit.
- **VHS video pipeline (NEW capability, scope step beyond static images):** Karen sends a **video**; the agent composites the VHS HUD onto it and posts a video. New `app/messaging/video.py` (ffmpeg: scale-to-fill+crop to 9:16, overlay a transparent HUD PNG, light grain, cap 60s → h264/yuv420p MP4). The HUD overlay is `app/templates/html/_vhs_overlay.html` (REC dot, timecode, yellow date stamp, scanlines, vignette) rendered transparent via a new `Renderer.render_file()` (omit_background). `on_demand` gained a `video/*` branch (`_preview_vhs_video`); `storage.upload_video` hosts the MP4; preview/edit/approve reuse the existing loop (caption edits keep the clip; `treatment="vhs_video"`). Per the approved concept the VHS post is full-frame **without** a Globex banner/logo — flagged below.
- **Acceptance PASS:** ruff + mypy clean (63 src files); full suite **129 passed / 46 skipped** — new `test_video.py` (ffmpeg arg builder + no-ffmpeg degrade + a gated real composite) and 4 e2e video cases (render→preview, video-only, caption-edit-keeps-clip, composite-failure-degrades), plus 6 renderer tests for the 2 static templates. **Live verified:** rendered both static templates (placeholder + photo); ran a real end-to-end VHS composite (testsrc clip → 1080×1920 h264 with the HUD burned in, frame inspected).
- **Deps / deploy:** `ffmpeg` is required for video — present locally; added `nixpacks.toml` (`aptPkgs=["ffmpeg"]`) so Railway installs it (Phase 8 completes the deploy config). Video posts capped 9:16 / 60s; v1 supports approve / cancel / caption-edit (re-styling the clip itself is a follow-up).
- **Open flags:** (1) the VHS video has **no Globex logo** (full-frame, per the client's explicit edit) — tension with the "logo on every post" rule; offered a subtle corner mark. (2) Video publishing via Blotato is still the Phase 5 stub (approve marks approved; real multi-platform video publish lands in Phase 5).
- **Queued next:** Phase 5 (Publishing) now spans images AND video.

### 2026-06-04 — Iteration: persistent memory + swipe-to-reply + Q&A
- **What landed:** the agent now remembers the whole conversation and can be replied-to like a person. New **`messages` table** (full transcript, both directions, with Twilio SID) is the backbone for two features. **Memory** (`app/ai/memory.py`): every AI call (intent, visual plan, generation, Q&A) is now prefixed with a context block = a **rolling summary** of older history + the **last ~25 messages** verbatim; `maybe_update_summary` folds scrolled-out messages into the summary (bounded cost as the thread grows). **Swipe-to-reply:** the webhook extracts Twilio's `OriginalRepliedMessageSid`; we map it via the `messages` log to the post it concerned and **re-open that post as the active draft**, so an edit redesigns it, an approval **reposts** it (re-rendered → back through approval, never auto-publish), and a question answers about it. **Conversational Q&A + analytics** (`app/ai/qa.py`): new `question` intent + `ANSWER` action; answers from memory + a recent-posts digest ("how many posts this month?", "what did we run for Ramadan?", "show me the Gulfood one" → re-sends that post's image).
- **Persistence for re-opening:** added `posts.render_meta` (JSONB) storing the full render inputs (display fields, treatment, image_prompt, raw image URL) at create + edit time, so an old post rebuilds into an exactly-editable draft. Transcript logging is low-friction: `twilio_client.send_text/send_media` auto-log every outbound (with the returned SID + optional post_id); inbound logged at the top of the handler. Logging is best-effort (never breaks a turn).
- **Acceptance PASS:** ruff + mypy clean (62 src files); full suite **115 passed / 46 skipped** — new `test_memory.py` (build_context format + summary trigger/skip), `test_state_machine.py` extended (now 28 = 4 states × 7 intents incl. `question`→`ANSWER`), and 6 new e2e cases (inbound logged with SID, question answered, question re-sends an image, swipe-reply edit, swipe-reply repost→publish, unknown-SID degrades). **Live verified:** schema migration applied to Supabase; the `messages` DB layer (create / oldest-first recent / by_sid / count / page) exercised live and cleaned up.
- **Twilio caveat:** reply context (`OriginalRepliedMessageSid`) is only attached for messages <7 days old and isn't explicitly guaranteed on the Sandbox — handled by graceful degrade (falls back to normal handling; memory still gives context). Verify the Sandbox actually sends it by logging a real quoted reply.
- **Queued next:** Phase 5 (Publishing via Blotato). Tool-based posts querying (vs the digest) is a future upgrade if history grows large.
